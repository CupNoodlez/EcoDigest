...
# We need to tell Python to look inside 'Model_Delivery_v1' if the package isn't in root
current_dir = Path(__file__).resolve().parent
delivery_dir = current_dir / "Model_Delivery_v1"

# Add both root and delivery dir to path to ensure we find the package
if str(current_dir) not in sys.path:
    sys.path.append(str(current_dir))
if delivery_dir.exists() and str(delivery_dir) not in sys.path:
    sys.path.append(str(delivery_dir))

# --- IMPORT MODEL ---
try:
    from climate_sentiment_pkg.classifier import ClimateSentimentClassifier
except ImportError:
    # Fallback: Try to find it if the user unzipped it differently
    st.error(f"❌ Could not find 'climate_sentiment_pkg'. \n\nExpected it in: `{current_dir}` \nOR in: `{delivery_dir}`")
    st.stop()

@st.cache_resource
def get_classifier():
    # Initialize the model (cached so it doesn't reload on every click)
    # We pass the specific path to model files if needed, or let it auto-detect
