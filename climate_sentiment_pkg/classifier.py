...
class ClimateSentimentClassifier:
    ...
    Directory dependent: Expects 'model_files' to be in the same directory as this script.
    """
    
    def __init__(self, model_dir: Optional[str] = None):
        # 1. DYNAMIC PATHING
        # If no path provided, look for 'model_files' in the same folder as this script
        if model_dir is None:
            current_script_path = Path(__file__).resolve().parent
            self.model_path = current_script_path / "model_files"
        else:
...
if __name__ == "__main__":
    ...
    clf = ClimateSentimentClassifier()
    
    test_phrase = "We need to stop using coal immediately."
    res = clf.predict(test_phrase)
